(function() {
	...
	function startup() {
		...
		for (var i = 0; i < friends.length; i++) {//for every friend in the friends list
			friends[i].addEventListener('click', function() {//save it to an array and to all of them add a click function
				...
				// Select friend
				this.className += ' active'; //make THIS object friendactive

				// Get notes for selected person
				selectedUserId = this.getAttribute('uid');//this specific user's id to the var selectedUserID
				var notes = getNotes(selectedUserId, function(notes) {//creates dynamic elements
	...
	function getNotes(userId, callback) {
		...
		var xhttp = new XMLHttpRequest();//initiates an object that can handle a request and a response
    ...
    document.addEventListener('DOMContentLoaded', startup);//executes start up way at the top! an event so doesn't need ()
})();//execute the anonymous function!
//functions inside don't execute because they're not called
